           ...
           case 'remove':
             if (chat.welcome) {
                ...
                for (let user of participants) { 
                    ...
                    if (action === 'add') {
                        if (user.includes(this.user.jid)) return this.reply(id, `Hello ${'@' + user.split('@')[0]}👋\n\nWelcome to Group ${groupMetadata.subject}`, m)                       
...
global.dfail = async (type, m, conn) => {
    ...
    if (msg) return conn.sendButtonImg(m.chat, set.fla + "acces denied", "\n*───「 ACCESS DENIED 」───*\n\n" + msg, set.wm, "menu", '/menu', m)
    let unreg = { 
        unreg: `Belum *Terdaftar,* Silahkan Daftar Dengan Mengetik *#daftar nama.umur*\n\nContoh: *#daftar ${m.name}.17*`
    }[type]
    if (unreg) return conn.sendButtonImg(m.chat, set.fla + "acces denied", "\n*───「 ACCESS DENIED 」───*\n\n" + unreg, set.wm, "menu", '/menu', m)
}
